...
DROP TRIGGER INFO_VENTA_ID;
DROP SEQUENCE INFO_VENTA_ID_SEQ;
...
CREATE SEQUENCE INFO_VENTA_ID_SEQ START WITH 1;
...
CREATE OR REPLACE TRIGGER INFO_VENTA_ID BEFORE INSERT ON INFO_VENTA
FOR EACH ROW

BEGIN
    SELECT INFO_VENTA_ID_SEQ.NEXTVAL INTO :new.id_venta FROM dual;
END;
/
ALTER TRIGGER INFO_VENTA_ID ENABLE;
...
BEGIN
    ...
    IF INSERTING THEN
        INSERT INTO INFO_VENTA(EMAIL_CLIENTE,CI_VENDEDOR,ID_PAPEL,PRECIO_FINAL,BONO_EMPLEADO,DESCUENTO_CLIENTE,FECHA)
        VALUES(:NEW.EMAIL_CLIENTE,:NEW.CI_VENDEDOR, :NEW.ID_PAPEL,v_precio_descuento, v_bonos_empleado, v_descuentos_cliente,:NEW.FECHA);
    END IF;
